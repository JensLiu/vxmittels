            ...
            for socket in sockets:
                ...
                for core in cores:
                    ...
                    for wid in wids:
                        ...
                        control_flow_divergence_points, data_flow_divergence_points = self.analyse_flow_by_id(cluster, socket, core, wid)
                        ...
                        if first_data_divergence:
                            ...
                            print(f"{id}: First data flow divergence at instruction {instr.uuid} (PC={instr.pc}):")
                            print(f"  Trace 1: pc={hex(instr.pc) if instr.pc else '???'} {instr.instruction_text} uuid=#{instr.uuid}")
                            print(f"  Trace 2: pc={hex(instr_cmp.pc) if instr_cmp.pc else '???'} {instr_cmp.instruction_text} uuid=#{instr_cmp.uuid}")
